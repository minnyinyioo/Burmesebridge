...
export default function KycApplicationPanel({ locale, userId, defaultExpanded = false }: { locale: string; userId: string; defaultExpanded?: boolean }) {
  const copy = locale === "zh" ? {
    ...
    terms: ["KYC 只用于确认你在 BurmeseBridge 私立学习平台内的身份、教师或作者申请，不是政府认证、学历认证、签证、银行或金融 KYC。", "申请资料必须真实、完整并属于本人。不得冒用身份、提交伪造或篡改文件。管理员可以要求补充资料或拒绝申请。", "平台只保存审核所需的最少信息。完整证件号码不会保存，只保存不可逆哈希和后 4–8 位；证件文件保存在私有存储中，仅申请人和授权 Admin 可查看。", "本平台使用自托管的人工审核流程，不会将证件资料发送给第三方身份验证服务。", "审核完成后，管理员可以按保留期限清理证件文件。你可以通过账户删除或隐私请求要求删除资料，但法律允许保留的审核记录除外。", "KYC 通过只表示 BurmeseBridge 内部审核通过，不代表任何政府教育机构、政府部门、学校、大学或职业资格机构。"],
