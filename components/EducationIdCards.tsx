...
import PhysicalCardApplication from "@/components/PhysicalCardApplication";
...
export default function EducationIdCards({ locale, userId }: { locale: string; userId: string }) {
  ...
  const copy = locale === "zh"
    ? { title: "我的教育证件", empty: "身份审核通过后，管理员可以签发学生证或教师证。", student: "学生证", teacher: "教师证", issued: "签发日期", expires: "有效期至", number: "证件编号", anti: "防伪码", verify: "在线查验", notice: "仅用于证明持有人在 BurmeseBridge 私立学习平台内的学习或教学身份，不是政府证件，也不代表政府教育机构认证。" }
    : locale === "my"
      ? { title: "ကျွန်ုပ်၏ ပညာရေးကတ်", empty: "Identity စစ်ဆေးအတည်ပြုပြီးပါက Admin က ကျောင်းသားကတ် သို့မဟုတ် ဆရာကတ် ထုတ်ပေးနိုင်သည်။", student: "ကျောင်းသားကတ်", teacher: "ဆရာကတ်", issued: "ထုတ်ပေးသည့်ရက်", expires: "သက်တမ်းကုန်ရက်", number: "ကတ်အမှတ်", anti: "အတုအပကာကွယ်ရေးကုဒ်", verify: "အွန်လိုင်းမှ စစ်ဆေးရန်", notice: "BurmeseBridge ပုဂ္ဂလိကသင်ယူရေးပလက်ဖောင်းအတွင်း သင်ယူသူ/သင်ကြားသူအဖြစ်သာ သက်သေပြသည်။ အစိုးရ ID မဟုတ်ပါ။" }
      : { title: "My education IDs", empty: "After identity approval, an administrator can issue a student or teacher ID.", student: "Student ID", teacher: "Teacher ID", issued: "Issued", expires: "Valid until", number: "Card number", anti: "Authenticity code", verify: "Verify online", notice: "This only confirms learning or teaching status within the private BurmeseBridge learning platform. It is not a government ID or government accreditation." };
  ...
  useEffect(() => {
    let active = true;
    void supabase.from("education_id_cards").select("id,card_type,card_no,verification_code,holder_name,issued_at,expires_at").eq("user_id", userId).eq("status", "active").order("issued_at", { ascending: false }).then(({ data }) => {
      if (active) setCards((data || []) as Card[]);
    });
    return () => { active = false; };
  }, [userId]);
