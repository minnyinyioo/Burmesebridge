...
export default function AccountSecurityPanel({ locale }: { locale: string }) {
  ...
  return <section className="account-security"><h2><ShieldCheck size={20} />{copy.title}</h2>
    ...
    <Dialog open={passwordOpen} onOpenChange={(open) => { setPasswordOpen(open); if (!open) setPasswords({ current: "", next: "", confirm: "" }); }}><DialogContent className="account-dialog"><DialogHeader><DialogTitle>{copy.password}</DialogTitle><DialogDescription>{copy.passwordHelp}</DialogDescription></DialogHeader><form onSubmit={changePassword} className="security-form">
