...
export default function HskAssessment({ locale }: { locale: string }) {
  ...
  const copy = locale === "zh" ? {
    eyebrow:"中文水平诊断",title:"HSK 1–6 综合能力测试",intro:"用 42 道听力、阅读、书写、词汇和语法题，全面了解你目前的中文水平。约需 25–35 分钟。",start:"开始测试",notice:"本测试用于学习诊断，不是官方 HSK 考试或证书。",progress:"测试进度",previous:"上一题",next:"下一题",submit:"查看结果",unanswered:"请先填写或选择答案",result:"你的诊断结果",level:"建议学习级别",score:"总正确率",correct:"答对",analysis:"分级表现",skillAnalysis:"专项能力",review:"错题解析",restart:"重新测试",learn:"去学习视频",saved:"成绩已保存到你的账号",guest:"登录后可保存每次测试记录",saveError:"成绩暂时无法保存，但不影响查看结果",pre:"HSK 1 入门准备",advice:"建议先从拼音、基础汉字和日常表达开始。",levelAdvice:"建议重点学习 HSK {level} 内容，并定期复测。",play:"播放中文录音",writePlaceholder:"请输入中文答案",download:"下载 PDF 成绩单",downloading:"正在生成 PDF…",pdfError:"PDF 生成失败，请重试"
  } : locale === "my" ? {
    eyebrow:"တရုတ်ဘာသာအဆင့် စစ်ဆေးမှု",title:"HSK အဆင့် ၁–၆ ဘက်စုံစွမ်းရည် စစ်ဆေးမှု",intro:"နားထောင်မှု၊ ဖတ်ရှုမှု၊ ရေးသားမှု၊ ဝေါဟာရနှင့် သဒ္ဒါ မေးခွန်း ၄၂ ခုဖြင့် တရုတ်ဘာသာစွမ်းရည်ကို စစ်ဆေးပါ။ ၂၅–၃၅ မိနစ်ခန့် ကြာပါမည်။",start:"စစ်ဆေးမှု စတင်ရန်",notice:"ဤစစ်ဆေးမှုသည် လေ့လာရေးဆိုင်ရာ အဆင့်သတ်မှတ်မှုသာဖြစ်ပြီး တရားဝင် HSK စာမေးပွဲ သို့မဟုတ် လက်မှတ်မဟုတ်ပါ။",progress:"ပြီးစီးမှု",previous:"ရှေ့မေးခွန်း",next:"နောက်မေးခွန်း",submit:"ရလဒ်ကြည့်ရန်",unanswered:"အဖြေကို ဦးစွာရေးပါ သို့မဟုတ် ရွေးပါ",result:"သင်၏ စစ်ဆေးမှုရလဒ်",level:"အကြံပြုလေ့လာမှုအဆင့်",score:"အဖြေမှန် ရာခိုင်နှုန်း",correct:"မှန်ကန်သည့်အဖြေ",analysis:"အဆင့်အလိုက် စွမ်းဆောင်ရည်",skillAnalysis:"စွမ်းရည်အလိုက် ရလဒ်",review:"အမှားများ ရှင်းလင်းချက်",restart:"ပြန်လည်စစ်ဆေးရန်",learn:"ဗီဒီယိုသင်ခန်းစာများ",saved:"ရလဒ်ကို သင့်အကောင့်တွင် သိမ်းဆည်းပြီးပါပြီ",guest:"ဝင်ရောက်ပြီးနောက် စစ်ဆေးမှုမှတ်တမ်းကို သိမ်းဆည်းနိုင်ပါသည်",saveError:"ရလဒ်ကို မသိမ်းဆည်းနိုင်သေးသော်လည်း ရလဒ်ကြည့်ရှုမှုကို မထိခိုက်ပါ",pre:"HSK 1 မတိုင်မီ အခြေခံအဆင့်",advice:"Pinyin၊ အခြေခံတရုတ်စာလုံးများနှင့် နေ့စဉ်သုံးစကားများမှ စတင်လေ့လာရန် အကြံပြုပါသည်။",levelAdvice:"HSK အဆင့် {level} အကြောင်းအရာကို အဓိကလေ့လာပြီး ပုံမှန်ပြန်လည်စစ်ဆေးပါ။",play:"တရုတ်အသံဖိုင် ဖွင့်ရန်",writePlaceholder:"တရုတ်စာဖြင့် အဖြေရေးပါ",download:"PDF ရလဒ်စာတမ်း ဒေါင်းလုဒ်ရန်",downloading:"PDF ဖန်တီးနေသည်…",pdfError:"PDF ဖန်တီးမရပါ။ ပြန်လည်ကြိုးစားပါ"
  } : {
    eyebrow:"Chinese level diagnostic",title:"HSK 1–6 comprehensive assessment",intro:"Assess listening, reading, writing, vocabulary and grammar with 42 questions. Takes about 25–35 minutes.",start:"Start test",notice:"This is a learning diagnostic, not an official HSK examination or certificate.",progress:"Progress",previous:"Previous",next:"Next",submit:"View result",unanswered:"Enter or choose an answer before continuing",result:"Your diagnostic result",level:"Recommended level",score:"Overall accuracy",correct:"Correct answers",analysis:"Level performance",skillAnalysis:"Skill performance",review:"Answer review",restart:"Try again",learn:"Learning videos",saved:"Result saved to your account",guest:"Sign in to save your assessment history",saveError:"The result could not be saved, but you can still view it",pre:"Pre-HSK 1 foundation",advice:"Start with Pinyin, basic characters and everyday expressions.",levelAdvice:"Focus on HSK {level} material and retake this diagnostic regularly.",play:"Play Chinese audio",writePlaceholder:"Type your answer in Chinese",download:"Download PDF report",downloading:"Generating PDF…",pdfError:"PDF generation failed. Please try again"
  };
  ...
  return <main className="hsk-page"><div ref={resultRef} className="hsk-report"><section className="hsk-result-head"><CheckCircle2 size={34}/><span>{copy.result}</span><h1>{result.estimatedLevel ? `HSK ${result.estimatedLevel}` : copy.pre}</h1><p>{result.estimatedLevel ? copy.levelAdvice.replace("{level}",String(result.estimatedLevel)) : copy.advice}</p><small>{copy.notice}</small></section><div className="hsk-result-grid"><article><span>{copy.level}</span><strong>{result.estimatedLevel ? `HSK ${result.estimatedLevel}` : "Pre-HSK"}</strong><small>CEFR-style: {result.cefr}</small></article><article><span>{copy.score}</span><strong>{result.score}%</strong><small>{copy.correct}: {result.correct}/{result.total}</small></article></div><section className="hsk-breakdown"><h2>{copy.analysis}</h2>{result.byLevel.map(item=><div key={item.level}><span>HSK {item.level}</span><div><i style={{width:`${item.rate*100}%`}}/></div><strong>{item.correct}/{item.total}</strong></div>)}</section><section className="hsk-breakdown"><h2>{copy.skillAnalysis}</h2>{result.bySkill.map(item=><div key={item.skill}><span>{item.skill}</span><div><i style={{width:`${item.rate*100}%`}}/></div><strong>{item.correct}/{item.total}</strong></div>)}</section>{wrong.length?<section className="hsk-review"><h2>{copy.review}</h2>{wrong.map(item=><details open key={item.id}><summary><span>HSK {item.level}</span>{item.prompt}</summary><p><b>✓ {typeof item.answer==="number"?item.options?.[item.answer]:item.answer}</b></p><p>{item.explanation}</p></details>)}</section>:null}</div><p className={`hsk-save-state ${saved}`}>{saved==="saved"?copy.saved:saved==="guest"?copy.guest:saved==="error"?copy.saveError:""}</p>{pdfState==="error"?<p className="hsk-hint hsk-pdf-error">{copy.pdfError}</p>:null}<div className="hsk-result-actions"><button className="hsk-secondary" onClick={start}><RotateCcw size={18}/>{copy.restart}</button><button className="hsk-secondary" disabled={pdfState==="working"} onClick={()=>void downloadPdf()}><Download size={18}/>{pdfState==="working"?copy.downloading:copy.download}</button><Link className="hsk-primary" href={`/${locale}/videos`}>{copy.learn}<ArrowRight size={18}/></Link></div></main>;
}
