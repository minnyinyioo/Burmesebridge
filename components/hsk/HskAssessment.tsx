...
export default function HskAssessment({ locale }: { locale: string }) {
  ...
  const [paper,setPaper] = useState<HskPresentedQuestion[]>(()=>createHskAssessmentPaper());
  ...
  async function finish(){
    ...
    setPhase("result");
    const scored = scoreHsk(answers,paper);
    const {data:{user}} = await supabase.auth.getUser();
    if (!user){setSaved("guest");return;}
    const displayName=reportName.trim()||user.user_metadata?.display_name||user.email?.split("@")[0]||"Learner";
    const questionIds=paper.map(item=>item.id);
    const {data,error} = await supabase.from("hsk_assessment_attempts").insert({user_id:user.id,display_name:displayName,question_ids:questionIds,estimated_level:scored.estimatedLevel,cefr_level:scored.cefr,score:scored.score,correct_answers:scored.correct,total_questions:scored.total,answers,level_breakdown:scored.byLevel,skill_breakdown:scored.bySkill}).select("report_code,created_at").single();
    if(error||!data){setSaved("error");return;}
    const code=String(data.report_code),createdAt=new Date(String(data.created_at));
    window.localStorage.setItem("bb:lastHskQuestionIds",JSON.stringify(questionIds));
    setReportMeta({id:code,date:formatReportDate(createdAt,locale),verificationUrl:`https://burmesebridge.com/${locale}/hsk-result/${encodeURIComponent(code)}`});
    setSaved("saved");
  }
  function advance(){if(answers[question.id] === undefined)return;if(index === paper.length-1){void finish();return;}setIndex(index+1);}

  if(phase === "intro") return <main className="hsk-page"><section className="hsk-hero"><span className="hsk-eyebrow"><ClipboardCheck size={18}/>{copy.eyebrow}</span><h1>{copy.title}</h1><p>{copy.intro}</p><label className="hsk-name-field">{copy.nameLabel}<input value={reportName} onChange={event=>setReportName(event.target.value)} maxLength={80} placeholder={copy.namePlaceholder}/><small>{copy.nameHelp}</small></label><div className="hsk-feature-row"><span>{hskQuestions.length} Questions</span><span>HSK 1–6</span><span>Listening · Reading · Writing</span><span>Vocabulary · Grammar</span></div><button className="hsk-primary" onClick={start}><Target size={19}/>{copy.start}</button><small>{copy.notice}</small></section></main>;

  if(phase === "test") {
    const selected = answers[question.id];
    return <main className="hsk-page"><section className="hsk-test-card"><header><div><span>{copy.progress}</span><strong>{index+1} / {paper.length}</strong></div><div className="hsk-progress"><i style={{width:`${(index+1)/paper.length*100}%`}}/></div></header><div className="hsk-question-meta"><span>HSK {question.level}</span><span>{question.skill}</span></div>{question.audioText?<button type="button" className="hsk-audio" onClick={playAudio}><Volume2 size={20}/>{copy.play}</button>:null}<h1>{question.prompt}</h1>{question.responseType==="text"?<input className="hsk-writing-input" lang="zh-CN" value={typeof selected==="string"?selected:""} placeholder={copy.writePlaceholder} onChange={event=>setAnswers({...answers,[question.id]:event.target.value})}/>:<div className="hsk-options">{question.options?.map((option,optionIndex)=><button className={selected===optionIndex?"selected":""} key={option} onClick={()=>setAnswers({...answers,[question.id]:optionIndex})}><b>{String.fromCharCode(65+optionIndex)}</b><span>{option}</span></button>)}</div>}{(selected===undefined||selected==="")&&index>0?<p className="hsk-hint">{copy.unanswered}</p>:null}<footer><button className="hsk-secondary" disabled={index===0} onClick={()=>setIndex(index-1)}><ArrowLeft size={18}/>{copy.previous}</button><button className="hsk-primary" disabled={selected===undefined||selected===""} onClick={advance}>{index===paper.length-1?copy.submit:copy.next}<ArrowRight size={18}/></button></footer></section></main>;
  }

  const wrong = paper.filter((item)=>!isHskAnswerCorrect(item,answers[item.id])).map((item)=>localizeHskQuestion(item,locale));
