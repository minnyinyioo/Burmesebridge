...
export default function HskAssessment({ locale }: { locale: string }) {
  ...
  return <main className="hsk-page">
    <div ref={resultRef} className="hsk-report">
      <header className="hsk-report-brand"><BrandLogo size={42}/><div><strong>{copy.result}</strong><span>burmesebridge.eu.cc</span></div></header>
      ...
      {wrong.length?<section className="hsk-review"><h2>{copy.review}</h2>{wrong.map(item=><details open key={item.id}><summary><span>HSK {item.level}</span>{item.prompt}</summary><p><b>✓ {typeof item.answer==="number"?item.options?.[item.answer]:item.answer}</b></p><p>{item.explanation}</p></details>)}</section>:null}
      <footer className="hsk-report-footer"><BrandLogo size={28}/><p>burmesebridge.eu.cc · admin@burmesebridge.eu.cc</p><small>{copy.notice}</small></footer>
    </div>
