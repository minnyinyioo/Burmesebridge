...
type Plan = { id:number; code:"monthly"|"yearly"|"lifetime"; price:number; currency:string; enabled:boolean; badge_type:string };
type Request = { id:number; user_id:string; payment_reference:string; proof_path:string|null; created_at:string; quoted_price:number|null; quoted_currency:string|null; quoted_days:number|null; quoted_badge:string|null; knowledge_membership_plans:{code:string}|null };

export default function MembershipManager({locale}:{locale:string}) {
  const copy = locale === "zh"
    ? { title:"全站会员方案", monthly:"月会员", yearly:"年会员", lifetime:"终身会员", save:"保存方案", enable:"启用", requests:"会员付款审核", empty:"暂无会员申请", approve:"确认开通", reject:"拒绝", proof:"查看凭证", positive:"启用的会员方案价格必须大于 0。", saved:"会员方案已保存。", quote:"订单快照", days:"天", unlimited:"长期" }
    : locale === "my"
      ? { title:"ဝက်ဘ်ဆိုက် အဖွဲ့ဝင်ကြေး အစီအစဉ်များ", monthly:"လစဉ် အဖွဲ့ဝင်", yearly:"နှစ်စဉ် အဖွဲ့ဝင်", lifetime:"တစ်သက်တာ အဖွဲ့ဝင်", save:"ပြောင်းလဲမှုများကို သိမ်းဆည်းရန်", enable:"အသုံးပြုရန်", requests:"အဖွဲ့ဝင်ကြေး ပေးချေမှု စိစစ်ခြင်း", empty:"စိစစ်ရန် လျှောက်ထားချက် မရှိသေးပါ။", approve:"အတည်ပြုပြီး ဖွင့်ပေးရန်", reject:"ငြင်းပယ်ရန်", proof:"ငွေပေးချေမှု အထောက်အထားကို ကြည့်ရန်", positive:"အသုံးပြုမည့် အဖွဲ့ဝင်ကြေး အစီအစဉ်၏ ဈေးနှုန်းသည် သုညထက် ကြီးရမည်။", saved:"အဖွဲ့ဝင်ကြေး အစီအစဉ်များကို သိမ်းဆည်းပြီးပါပြီ။", quote:"အော်ဒါအချက်အလက်", days:"ရက်", unlimited:"ရေရှည်" }
      : { title:"Site-wide memberships", monthly:"Monthly", yearly:"Annual", lifetime:"Lifetime", save:"Save plans", enable:"Enabled", requests:"Membership payment review", empty:"No membership requests", approve:"Approve", reject:"Reject", proof:"View proof", positive:"The price of every enabled membership plan must be greater than zero.", saved:"Membership plans saved.", quote:"Order snapshot", days:"days", unlimited:"Long-term" };
  const [plans,setPlans] = useState<Plan[]>([]);
  const [requests,setRequests] = useState<Request[]>([]);
  const [message,setMessage] = useState("");
  const load = useCallback(async () => {
    const [{data:p,error:pError},{data:r,error:rError}] = await Promise.all([
      supabase.from("knowledge_membership_plans").select("id,code,price,currency,enabled,badge_type").order("sort_order"),
      supabase.from("knowledge_membership_requests").select("id,user_id,payment_reference,proof_path,created_at,quoted_price,quoted_currency,quoted_days,quoted_badge,knowledge_membership_plans(code)").eq("status","pending").order("created_at"),
    ]);
  ...
  async function save() {
    ...
    const results = await Promise.all(plans.map((plan) => supabase.from("knowledge_membership_plans").update({price:plan.price,currency:plan.currency,enabled:plan.enabled,badge_type:plan.badge_type,updated_at:new Date().toISOString()}).eq("id",plan.id)));
    const error = results.find((result) => result.error)?.error;
    setMessage(error?.message || copy.saved);
  }
  async function review(id:number,status:"approved"|"rejected") {
    if(!window.confirm(locale==="zh"?"确认此会员订单审核结果？":"Confirm this membership review?"))return;
    const {error} = await supabase.from("knowledge_membership_requests").update({status}).eq("id",id).eq("status","pending");
  ...
  return <section className="membership-admin">
    ...
    <div className="membership-plan-admin">{plans.map((plan,index) => <article key={plan.id}>
      ...
      <label>{locale==="zh"?"会员徽章":"Membership badge"}<select value={plan.badge_type} onChange={e=>setPlans(current=>current.map((item,i)=>i===index?{...item,badge_type:e.target.value}:item))}><option value="member">Member</option><option value="vip">VIP</option><option value="premium">Premium</option></select></label>
