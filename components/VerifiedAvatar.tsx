...
export default function VerifiedAvatar({name,avatarUrl,verified,size=52}:{name:string;avatarUrl?:string|null;verified?:boolean|null;size?:number}) {
  ...
  return <span className="verified-avatar" style={{width:size,height:size}} aria-label={verified?name+", verified":name}>
    ...
    {verified ? <span className={`verified-avatar-mark${showVerified?" is-open":""}`} style={{width:markSize,height:markSize,right:4,bottom:4}} role="button" tabIndex={0} aria-label="已认证 / Verified" data-tooltip="已认证 / Verified" onClick={(event)=>{event.stopPropagation();setShowVerified(value=>!value)}} onKeyDown={(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();setShowVerified(value=>!value)} if(event.key==="Escape")setShowVerified(false)}}><Check size={Math.max(10,Math.round(markSize*.62))} strokeWidth={3}/></span> : null}
  </span>;
}
