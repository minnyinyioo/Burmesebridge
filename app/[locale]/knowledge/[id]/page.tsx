...
import { ArrowLeft, Check, CheckCircle2, Circle, Download, FileText, Flame, ListVideo, LockKeyhole, PlayCircle, Target, UserRound, Users, Languages } from "lucide-react";
...
type Product = { id:number; title_my:string|null; title_zh:string|null; title_en:string|null; description_my:string|null; description_zh:string|null; description_en:string|null; preview_youtube_id:string|null; price:number; currency:string; level:string|null; skill:string|null; teacher_name:string|null; teacher_bio:string|null; learning_objectives:string[]|null; target_audience:string[]|null; estimated_minutes:number|null };
...
type LessonContent = { lesson_id:number; body_my:string|null; body_zh:string|null; body_en:string|null; youtube_id:string|null; captions:unknown; vocabulary:unknown; handout_my:string|null; handout_zh:string|null; handout_en:string|null };
...
export default function CoursePage() {
  ...
  const copy = locale === "zh"
    ? { back:"返回课程", syllabus:"课程目录", preview:"免费试看", locked:"购买后解锁", empty:"课程还没有添加课时", missing:"课程不存在或已下架", complete:"标记完成", completed:"已完成", progress:"学习进度", loginProgress:"登录后可保存进度", select:"从右侧课程表选择课时", intro:"课程介绍", unlocked:"课程已解锁", teacher:"授课教师", objectives:"学习目标", audience:"适用对象", captions:"字幕", vocabulary:"重点词汇", handout:"课程讲义" }
    : locale === "my"
      ? { back:"သင်တန်းများသို့", syllabus:"သင်ခန်းစာစာရင်း", preview:"အခမဲ့အစမ်း", locked:"ဝယ်ပြီးမှ ဖွင့်မည်", empty:"သင်ခန်းစာ မရှိသေးပါ", missing:"သင်တန်းမရှိပါ", complete:"ပြီးဆုံးကြောင်း မှတ်မည်", completed:"ပြီးဆုံး", progress:"သင်ယူမှု", loginProgress:"ဝင်ပြီး တိုးတက်မှုသိမ်းပါ", select:"စာရင်းမှ သင်ခန်းစာရွေးပါ", intro:"သင်တန်းအကြောင်း", unlocked:"သင်တန်းဖွင့်ပြီး", teacher:"သင်ကြားသူ", objectives:"သင်ယူမှုရည်မှန်းချက်", audience:"သင့်တော်သူများ", captions:"စာတန်းထိုး", vocabulary:"အဓိကဝေါဟာရ", handout:"သင်ခန်းစာမှတ်စု" }
      : { back:"Back to courses", syllabus:"Course syllabus", preview:"Free preview", locked:"Unlock after purchase", empty:"No lessons have been added", missing:"Course not found", complete:"Mark complete", completed:"Completed", progress:"Progress", loginProgress:"Sign in to save progress", select:"Choose a lesson from the syllabus", intro:"About this course", unlocked:"Course unlocked", teacher:"Instructor", objectives:"Learning objectives", audience:"Who this is for", captions:"Captions", vocabulary:"Key vocabulary", handout:"Lesson handout" };
  ...
  const load = useCallback(async () => {
    ...
    const [{data:p},{data:l},{data:c},{data:a},{data:s},{data:auth}] = await Promise.all([
      supabase.from("knowledge_products").select("id,title_my,title_zh,title_en,description_my,description_zh,description_en,preview_youtube_id,price,currency,level,skill,teacher_name,teacher_bio,learning_objectives,target_audience,estimated_minutes").eq("id",id).eq("status","published").maybeSingle(),
      ...
      supabase.from("knowledge_lesson_content").select("lesson_id,body_my,body_zh,body_en,youtube_id,captions,vocabulary,handout_my,handout_zh,handout_en"),
    ...
    if (auth.user) {
      const [{data:access},{data:membership},{data:request},{data:progress},{data:streakData}] = await Promise.all([
  ...
  const normalizeList=(value:unknown):string[]=>Array.isArray(value)?value.map((item)=>typeof item==="string"?item:JSON.stringify(item)).filter(Boolean):[];
  const handout=selectedContent?(locale==="zh"?selectedContent.handout_zh||selectedContent.handout_my||selectedContent.handout_en:locale==="en"?selectedContent.handout_en||selectedContent.handout_my||selectedContent.handout_zh:selectedContent.handout_my||selectedContent.handout_zh||selectedContent.handout_en):null;
  ...
  return <main className="course-detail course-classroom">
    ...
    <header className="course-titlebar"><div><span>{accessible && Number(product.price) > 0 ? copy.unlocked : Number(product.price) === 0 ? "Free" : `${Number(product.price).toLocaleString()} ${product.currency}`}</span><h1>{localize(product,"title")}</h1><p>{localize(product,"description")}</p><div className="course-title-meta">{product.level?<span>{product.level}</span>:null}{product.skill?<span>{product.skill}</span>:null}{product.estimated_minutes?<span>{product.estimated_minutes} min</span>:null}</div></div></header>
    {(product.teacher_name||product.learning_objectives?.length||product.target_audience?.length)?<section className="course-overview-grid">{product.teacher_name?<article><UserRound/><div><strong>{copy.teacher}</strong><h2>{product.teacher_name}</h2>{product.teacher_bio?<p>{product.teacher_bio}</p>:null}</div></article>:null}{product.learning_objectives?.length?<article><Target/><div><strong>{copy.objectives}</strong><ul>{product.learning_objectives.map((item)=><li key={item}>{item}</li>)}</ul></div></article>:null}{product.target_audience?.length?<article><Users/><div><strong>{copy.audience}</strong><ul>{product.target_audience.map((item)=><li key={item}>{item}</li>)}</ul></div></article>:null}</section>:null}
    <section className="course-workspace">
      <div className="course-player-column">
        ...
        <article className="course-current-lesson">
          ...
          {selectedContent&&normalizeList(selectedContent.captions).length?<section className="lesson-resource-block"><h3><Languages size={17}/>{copy.captions}</h3>{normalizeList(selectedContent.captions).map((item,index)=><p key={`${index}-${item}`}>{item}</p>)}</section>:null}
          {selectedContent&&normalizeList(selectedContent.vocabulary).length?<section className="lesson-resource-block"><h3><FileText size={17}/>{copy.vocabulary}</h3><ul>{normalizeList(selectedContent.vocabulary).map((item,index)=><li key={`${index}-${item}`}>{item}</li>)}</ul></section>:null}
          {handout?<section className="lesson-resource-block"><h3><FileText size={17}/>{copy.handout}</h3><p>{handout}</p></section>:null}
        ...
        {selectedLesson&&selectedContent?<QuizPanel locale={locale} lessonId={selectedLesson.id} userId={userId}/>:null}
      </div>
      <aside className="course-syllabus">
        ...
        <div className="course-progress"><div><strong>{copy.progress}</strong><span>{progressValue}%</span></div><div><i style={{width:`${progressValue}%`}}/></div>{userId?<small className="course-streak"><Flame size={14}/>{locale==="zh"?"连续学习":locale==="my"?"ဆက်တိုက်လေ့လာမှု":"Study streak"} · {streak}</small>:<small>{copy.loginProgress}</small>}</div>
