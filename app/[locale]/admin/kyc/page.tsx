...
function KycAdminContent() {
  ...
  const copy = locale === "zh" ? { title: "KYC 身份审核", intro: "审查结构化身份资料和私有证件文件。最终决定只属于 Admin；系统不会自动批准。", pending: "待审核", in_review: "审核中", approved: "已通过", rejected: "已拒绝", expired: "已过期", empty: "暂无 KYC 申请", applicant: "申请人", kind: "申请类型", identity: "身份资料", document: "证件", address: "地址", consent: "条款版本", reviewNote: "审核说明；拒绝时必填", approve: "通过", reject: "拒绝", clear: "清理证件文件", cleared: "证件文件已清理", view: "查看私有文件", reason: "请填写拒绝原因", protected: "证件链接为短期签名链接；完整证件号码不会展示或保存。", student: "学生", teacher: "教师", author: "作者", company: "企业" } : locale === "my" ? { title: "KYC အထောက်အထား စစ်ဆေးခြင်း", intro: "ဖွဲ့စည်းထားသော ကိုယ်ရေးအချက်အလက်နှင့် private စာရွက်စာတမ်းများကို စစ်ဆေးပါ။ နောက်ဆုံးဆုံးဖြတ်ချက်သည် Admin သာ ဖြစ်ပြီး အလိုအလျောက် အတည်မပြုပါ။", pending: "စစ်ဆေးရန်", in_review: "စစ်ဆေးနေသည်", approved: "အတည်ပြု", rejected: "ပယ်ချ", expired: "သက်တမ်းကုန်", empty: "KYC လျှောက်ထားမှု မရှိပါ", applicant: "လျှောက်ထားသူ", kind: "အမျိုးအစား", identity: "ကိုယ်ရေးအချက်အလက်", document: "စာရွက်စာတမ်း", address: "လိပ်စာ", consent: "စည်းမျဉ်းဗားရှင်း", reviewNote: "စစ်ဆေးချက် (ပယ်ချလျှင် မဖြစ်မနေ)", approve: "အတည်ပြု", reject: "ပယ်ချ", clear: "စာရွက်စာတမ်း ဖျက်ရန်", cleared: "စာရွက်စာတမ်း ဖျက်ပြီးပါပြီ", view: "Private ဖိုင်ကြည့်ရန်", reason: "ပယ်ချရသည့်အကြောင်းပြချက် ရေးပါ", protected: "ဖိုင်လင့်ခ်သည် ကာလတို signed link ဖြစ်သည်။ အထောက်အထားနံပါတ်အပြည့်ကို မပြပါ သို့မဟုတ် မသိမ်းပါ။", student: "ကျောင်းသား", teacher: "ဆရာ", author: "စာရေးသူ", company: "ကုမ္ပဏီ" } : { title: "KYC identity review", intro: "Review structured identity data and private document files. Admin makes the final decision; the system never auto-approves.", pending: "Pending", in_review: "In review", approved: "Approved", rejected: "Rejected", expired: "Expired", empty: "No KYC applications", applicant: "Applicant", kind: "Application", identity: "Identity data", document: "Document", address: "Address", consent: "Consent version", reviewNote: "Review note; required when rejecting", approve: "Approve", reject: "Reject", clear: "Clear document files", cleared: "Document files cleared", view: "View private file", reason: "Enter a rejection reason", protected: "Document links are short-lived signed URLs; the full document number is never displayed or stored.", student: "Student", teacher: "Teacher", author: "Author", company: "Company" };

  const load = useCallback(async () => {
    setMessage("");
    const { data, error } = await supabase.from("kyc_verifications").select("id,user_id,application_kind,legal_name,date_of_birth,nationality,country,address,document_type,document_last4,document_front_path,document_back_path,status,review_note,consent_version,consented_at,retention_until,created_at,reviewed_at,profiles!kyc_verifications_user_id_fkey(display_name,email)").eq("status", filter).order("created_at", { ascending: filter === "pending" });
