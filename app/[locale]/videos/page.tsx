...
export default function VideosPage() {
  ...
  return <main className="video-page"><header className="video-page-head"><span><Video size={22} /></span><div><h1>{copy.title}</h1><p>{copy.intro}</p></div></header><Link className="hsk-video-banner" href={`/${locale}/hsk-test`}><span><ClipboardCheck size={24}/></span><div><strong>{copy.test}</strong><p>{copy.testCopy}</p></div><b>{copy.begin}<ArrowRight size={17}/></b></Link>
