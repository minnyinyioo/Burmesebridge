...
export default function HskVerifiedResult(){
  ...
  return <main className="hsk-page"><article className="hsk-report hsk-public-report"><header className="hsk-report-brand"><span className="hsk-report-lockup"><Image src="/brand-icon-1024.png" width={48} height={48} alt="BurmeseBridge"/><strong>Burmese<span>Bridge</span></strong></span><div><strong>{copy.title}</strong><span>burmesebridge.eu.cc</span></div></header><div className="hsk-verified-banner"><CheckCircle2 size={22}/><p><strong>{copy.valid}</strong><span>{result.report_code}</span></p></div><div className="hsk-result-grid"><article><span>{copy.level}</span><strong>{result.estimated_level?`HSK ${result.estimated_level}`:"Pre-HSK"}</strong><small>CEFR-style: {result.cefr_level}</small></article><article><span>{copy.score}</span><strong>{result.score}%</strong><small>{copy.correct}: {result.correct_answers}/{result.total_questions}</small></article></div>{rows(result.level_breakdown||[],"level")}{rows(result.skill_breakdown||[],"skill")}<footer className="hsk-report-footer"><ShieldCheck size={20}/><p>{copy.date}: {new Date(result.created_at).toLocaleString(locale)}</p><small>admin@burmesebridge.eu.cc · This is a learning diagnostic, not an official HSK certificate.</small></footer></article></main>;
}
